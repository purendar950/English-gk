const TOPICS=[
["vocabulary","Vocabulary","Word meanings, usage & SSC vocabulary"],
["synonyms","Synonyms","Similar meaning words"],
["antonyms","Antonyms","Opposite meaning words"],
["idioms","Idioms & Phrases","Meaning and usage"],
["one-word-substitution","One Word Substitution","Replace phrases with one word"],
["spelling","Spelling","Correctly spelt words"],
["error-detection","Error Detection","Grammar error identification"],
["sentence-improvement","Sentence Improvement","Improve the underlined part"],
["fill-blanks","Fill in the Blanks","Context-based grammar & vocabulary"],
["cloze-test","Cloze Test","Passage-based questions"],
["active-passive","Active & Passive Voice","Voice transformation"],
["narration","Direct & Indirect Speech","Narration transformation"],
["miscellaneous","Miscellaneous","Mixed SSC English questions"]
];
...
async function load(){
  ...
  await Promise.all(TOPICS.map(async([id,name])=>{
    try{
      ...
      list.filter(q=>q&&Array.isArray(q.options)&&q.options.length>1)
        .forEach(q=>all.push({...q,topic:id,topicName:name}));
    }catch(e){console.error("Question file failed:",id,e);}
  }));
  state.questions=all;
...
function renderQ(){
  ...
  $("#options").innerHTML=(q.options||q.opts||[]).map((o,i)=>
    `<button class="option" data-i="${i}">${String.fromCharCode(65+i)}. ${esc(o)}</button>`
  ).join("");
...
function finish(){
  ...
  $("#review").innerHTML=state.pool.map((q,i)=>
    `<div><b>Q${i+1}</b> — ${esc(q.question||q.q||"")}</div>`
  ).join("");
  show("result");
}
